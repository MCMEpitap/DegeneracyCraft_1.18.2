...
public class BasicPowerSteamGeneratorStructure {
    ...
    public static void hologram(Level level, BlockPos pos, BlockState state, BasicPowerSteamGeneratorBlockEntity blockEntity) {
        ...
        if (!displayHologram) {
            // スロットにアイテムがない場合、ホログラムを削除
            removeHolograms(level, basePos, facing, blockMapping);
            return;
        }

        // 段階的にホログラムを表示（最下段から上段へ）
        for (int y = maxY; y >= minY; y--) {
            int arrayY = maxY - y; // 配列インデックスの計算

            // 現在の段が正しく設置されているか確認
            if (!isLayerFormed(level, basePos, structureH0[arrayY], y, facing, blockMapping)) {
                // 正しく設置されていない段を表示
                displayLayer(level, basePos, structureH0[arrayY], y, facing, blockMapping);
                return; // 最初に見つかった段だけ表示して終了
            }
        }
    }

    /**
     * 指定された段が正しく設置されているかを判定
     */
    private static boolean isLayerFormed(Level level, BlockPos basePos, String[][] structureLayer, int yOffset, Direction facing, Map<String, Supplier<Block>> blockMapping) {
        for (int z = 0; z < structureLayer.length; z++) {
            for (int x = 0; x < structureLayer[z].length; x++) {
                String expectedBlock = structureLayer[z][x];
                if (" ".equals(expectedBlock)) continue;

                BlockPos relativePos = getRelativePos(basePos, x - 1, yOffset, z - 2, facing);
                BlockState targetState = level.getBlockState(relativePos);

                // ブロックが一致していない場合 false
                Supplier<Block> expectedSupplier = blockMapping.get(expectedBlock);
                if (expectedSupplier == null || !targetState.is(expectedSupplier.get())) {
                    return false;
                }
            }
        }
...
//
//        // 段階的にホログラムを表示（最下段から上段へ）
//        for (int y = minY; y <= maxY; y++) { // 最下段から処理
//            int arrayY = maxY - y; // 最下段が配列の最大インデックスに対応
//
//            // 現在の段が正しく設置されているか確認
