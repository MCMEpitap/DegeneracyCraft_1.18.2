...
public class PulverizerMenu extends AbstractContainerMenu {
    ...
    public PulverizerMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level.getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(5));
    }

    public PulverizerMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(DCMenuTypes.PULVERIZER_MENU.get(), pContainerId);
        checkContainerSize(inv, 3);
        blockEntity = ((PulverizerBlockEntity) entity);
        ...
        this.blockEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(handler -> {
            ...
            this.addSlot(new DCResultSlot(handler, 2, 116, 30));
        });
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, DCBlocks.PULVERIZER_BLOCK.get());
    }
