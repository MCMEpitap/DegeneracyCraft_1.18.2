...
public class BasicPerformanceBioReactorBusEnergyStorage implements IEnergyStorage {
    protected PortWorkBlockEntity pipe;
    protected Direction side;
    protected float lastReceived;

    public BasicPerformanceBioReactorBusEnergyStorage(PortWorkBlockEntity pipe, Direction side) {
        this.pipe = pipe;
        this.side = side;
    }
    ...
    public void tick() {
        if (this.pipe.getLevel().getGameTime() - this.lastReceived > 1F) {
            BasicPerformanceBioReactorBusType.INSTANCE.extractEnergy(this.pipe, this.side);
        }
    }
    ...
    public float receiveEnergyFloat(float maxReceive, boolean simulate) {
        ...
        return BasicPerformanceBioReactorBusType.INSTANCE.receiveEnergy(this.pipe, this.side, maxReceive, simulate);
    }
    ...
    public int receiveEnergy(int maxReceive, boolean simulate) {
        this.lastReceived = this.pipe.getLevel().getGameTime();
        return (int) BasicPerformanceBioReactorBusType.INSTANCE.receiveEnergy(this.pipe, this.side, maxReceive, simulate);
    }
