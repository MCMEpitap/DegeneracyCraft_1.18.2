...
public class BasicPerformanceOreSorterBusEnergyStorage implements IEnergyStorage {
    protected PortWorkBlockEntity pipe;
    protected Direction side;
    protected float lastReceived;

    public BasicPerformanceOreSorterBusEnergyStorage(PortWorkBlockEntity pipe, Direction side) {
        this.pipe = pipe;
        this.side = side;
    }
    ...
    public void tick() {
        if (this.pipe.getLevel().getGameTime() - this.lastReceived > 1F) {
            BasicPerformanceOreSorterBusType.INSTANCE.extractEnergy(this.pipe, this.side);
        }
    }
    ...
    public float receiveEnergyFloat(float maxReceive, boolean simulate) {
        ...
        return BasicPerformanceOreSorterBusType.INSTANCE.receiveEnergy(this.pipe, this.side, maxReceive, simulate);
    }
    ...
    public int receiveEnergy(int maxReceive, boolean simulate) {
        this.lastReceived = this.pipe.getLevel().getGameTime();
        return (int) BasicPerformanceOreSorterBusType.INSTANCE.receiveEnergy(this.pipe, this.side, maxReceive, simulate);
    }
