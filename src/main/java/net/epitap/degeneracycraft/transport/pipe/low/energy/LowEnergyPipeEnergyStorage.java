...
public class LowEnergyPipeEnergyStorage implements IEnergyStorage {
    protected PipeWorkBlockEntity pipe;
    protected Direction side;
    protected float lastReceived;

    public LowEnergyPipeEnergyStorage(PipeWorkBlockEntity pipe, Direction side) {
        this.pipe = pipe;
        this.side = side;
    }
    ...
    public void tick() {
        if (this.pipe.getLevel().getGameTime() - this.lastReceived > 1F) {
            LowEnergyPipeType.INSTANCE.extractEnergy(this.pipe, this.side);
        }
    }
    ...
    public float receiveEnergyFloat(float maxReceive, boolean simulate) {
        ...
        return LowEnergyPipeType.INSTANCE.receiveEnergy(this.pipe, this.side, maxReceive, simulate);
    }
    ...
    public int receiveEnergy(int maxReceive, boolean simulate) {
        this.lastReceived = this.pipe.getLevel().getGameTime();
        return (int) LowEnergyPipeType.INSTANCE.receiveEnergy(this.pipe, this.side, maxReceive, simulate);
    }
